...
public class TestActivity extends Activity {
		...
		clickBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (currentUser != null) {
					...
					if (clickBtn.getText().toString()
							.equals(CANCEL_PRAISE_ACTY_PHOTO)) {
						clickBtn.setText(LOADING);
						// 对活动照片取消赞
	...
	public boolean isNetLoad() {
		SharedPreferences sp = getSharedPreferences(
				Constants.ACTIVITY_CACHE_SP, MODE_PRIVATE);
			...
			public void callback(List<ObjActivity> objects, AVException e) {
				...
				if (objects != null && objects.size() > 0) {
					...
					for (ObjActivity activity : objects) {
						if (activity.getObjectId().equals(
								"55e2b25f60b2719ea5e22c96")) {
							...
							bean.setActivityCover(activity.getActivityCover()
									.getUrl());
							bean.setLocationAddress(activity
									.getLocationAddress());
					...
					editor.putString(Constants.ACTIVITY_CACHE_TIME,
	...
	public void initActuView(boolean isLocal) {
		...
		if (isLocal) {
			...
			followTv.setText("follow  "
					+ String.valueOf(bean.getOrderAndFollow()));
	...
	public void queryActyCovers(ObjActivity activity) {
		ObjActivityCoverWrap.queryActivityCover(activity,
				new ObjActivityCoverCallback() {
					...
					public void callback(List<ObjActivityCover> objects,
						...
						if (objects != null && objects.size() > 0) {
							...
							actyCoverTv.setText("cover:"
									+ String.valueOf(coverList.size()));
						}
					}
				});
	}
					...
					public void callback(List<ObjUser> objects, AVException e) {
						...
						if (objects != null) {
							...
							if (userList.size() > 0) {
								...
								if (userList.size() > 0) {
									orderUserTv.setText("users"
											+ userList.size()
											+ userList.get(0).getProfileClip()
													.getUrl());
								} else {
									orderUserTv.setText("users"
											+ userList.size());
								...
								int followCount = actyList.get(0)
										.getOrderAndFollow();
								followTv.setText("follow  "
										+ String.valueOf(followCount));
								/**
								 * 此处执行更新活动信息关注参加活动的人count字段
								 * */
								actyDao.updateOrderFollow(user.getObjectId(),
										0, 10);
							}
						}
					}
	...
	private void getInfo(ObjActivity activitySign) {
		// 查询是否报名
		ObjActivityWrap.queryUserJoin(activitySign, user,
				new ObjFunBooleanCallback() {
	...
	public void cancelPraiseActivity(ObjActivity acty) {
		ObjPraiseWrap.cancelPraiseActivity(user, acty,
				new ObjFunBooleanCallback() {
	...
	public void loadActvityPhoto(ObjActivity acty) {
		ObjActivityPhotoWrap.queryActivityPhotos(acty,
				new ObjActivityPhotoCallback() {
					...
					public void callback(List<ObjActivityPhoto> objects,
						...
						if (objects != null && objects.size() > 0) {
							...
							// 查询是否对照片点赞
							ObjActivityPhotoWrap.queryPhotoPraise(photoBean,
									user, new ObjFunBooleanCallback() {
	...
	public void praiseActivityPhoto(ObjActivityPhoto photo) {
		ObjActivityPhotoWrap.praiseActivityPhoto(photo, user,
				new ObjFunBooleanCallback() {
	...
	public void cancelPraiseActtyPhoto(ObjActivityPhoto photo) {
		ObjActivityPhotoWrap.cancelPraiseActivityPhoto(user, photo,
				new ObjFunBooleanCallback() {
	...
	public void activityFeedback(ObjActivity acty) {
		ObjActivityWrap.activityFeedBack(acty, user, "hello error",
				new ObjFunBooleanCallback() {
	...
	public void forgetPsdSms(String phoneNum) {
		ObjUserWrap.requestSmsCodeForResetPasswd(phoneNum,
				new ObjFunBooleanCallback() {
		...
		ObjUserWrap.resetPasswd(smsNum, newPsd, new ObjFunBooleanCallback() {
			...
			public void callback(boolean result, AVException e) {
				...
				if (result) {
					clickBtn.setText(LOAD_SUC);
					// 修改成功
					Toast.makeText(getApplicationContext(), "reset suc", 1000)
							.show();
				}
			}
		});
	...
	public void completeInfo(final ObjUser user) {
		...
		try {
			...
			fFile = AVFile.withAbsoluteLocalPath(Constants.HEAD_FILE_RECT
					+ user.getObjectId() + Constants.IMG_TYPE, fPath);
			yFile = AVFile.withAbsoluteLocalPath(Constants.HEAD_FILE_CIRCLE
					+ user.getObjectId() + Constants.IMG_TYPE, yPath);
