...
public class UserShieldDao {
	...
	public boolean queryIsShield(String userId,String shieldUserId){
		SQLiteDatabase sdb=helper.getWritableDatabase();
		Cursor cursor=sdb.rawQuery("select * from "+ Constants.USER_SHIELD_TB+" where "+Constants.USERID +"=? and "
				+Constants.SHIELD_USER_ID+"=?",new  String[]{userId,shieldUserId});
	...
	public void deleteByUser(String userId){
		SQLiteDatabase sdb=helper.getWritableDatabase();
		sdb.delete(Constants.USER_SHIELD_TB, Constants.USERID+"=?", new String[]{userId});
		sdb.close();
	}
}
