...
public class ObjUserWrap {
	 ...
	 *            回调对象
	 */
	public static void phoneIsAlreadyRegister(String phone,
			final ObjFunEnumCallback callback) {
		if (callback != null) {
	 ...
	 *            回调对象
	 */
	public static void login(String phone, String passwd,
			final ObjFunObjectCallback callback) {
		if (callback != null) {
			ObjUser.loginByMobilePhoneNumberInBackground(phone, passwd,
					new LogInCallback<AVUser>() {
						...
						public void done(AVUser avUser, AVException e) {
							if (avUser != null) {
								...
								if (e.getCode() == 210) {
									callback.callback(null, new AVException(1,
											"密码不正确"));
								} else {
									callback.callback(null, new AVException(2,
											"用户不存在"));
								}
							}
						}
	 ...
	 * 
	 */
	public static void register(final String phone, final String passwd,
			String smsCode, final ObjFunBooleanCallback callback) {
		if (callback != null) {
								...
								user.signUpInBackground(new SignUpCallback() {
									...
									public void done(AVException e) {
										if (e == null) {
											...
											log.e("注册失败异常", e.toString() + " "
													+ e.getCode());

											callback.callback(false,
													new AVException(1, "注册失败"));
										}
									}
								});
							} else {
								callback.callback(false, new AVException(0,
										"短信验证错误"));
							}
						}
					});
	 ...
	 *            回调对象
	 */
	public static void completeUserInfo(ObjUser user,
	 ...
	 *            回调对象
	 */
	public static void requestSmsCodeForResetPasswd(String phone,
			final ObjFunBooleanCallback callback) {
		log.e("zcq", "phone" + phone);
		if (callback != null) {
			ObjUser.requestPasswordResetBySmsCodeInBackground(phone,
	 ...
	 *            回调对象
	 */
	public static void resetPasswd(String smsCode, String newPasswd,
			final ObjFunBooleanCallback callback) {
		if (callback != null) {
			ObjUser.resetPasswordBySmsCodeInBackground(smsCode, newPasswd,
	 ...
	 * 根据用户ID获取用户信息
	 * */
	public static void getObjUser(String objId,
			final ObjUserInfoCallback callback) {
		// 查询用户，你需要使用特殊的用户查询对象来完成
		AVQuery<AVUser> query = AVUser.getQuery();
