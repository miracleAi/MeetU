...
public class MiliaoChannelFragment extends Fragment implements OnClickListener{
	...
	//		conv = MyApplication.chatClient.getConversation(""+seekChatBean.getCreator());
			initView();
...
//			getMembers(conv);
	...
	private void initView() {
		...
		if(seekChatBean.getCreator().getProfileClip()!=null){
			log.e("zcq", "设置头像");
			finalBitmap.display(photoManager, seekChatBean.getCreator().getProfileClip().getUrl());
...
//		 */
							...
							if(memberUserList.size()==list.size()){
								...
								if(list.size()>=5){
									...
									if(memberUserList.get(2).getProfileClip().getUrl()!=null){
										...
										threeUser.setOnClickListener(MiliaoChannelFragment.this); 
									}
	...
	public void onClick(View v) {
		...
		case R.id.two_photoHead_channel_img:
			...
			log.e("lucifer", "userId0=="+seekChatBean.getMembers().get(1).get("userId"));
			two.putExtra("userId", ""+seekChatBean.getMembers().get(1).get("userId"));
			startActivity(two);
			break;
		case R.id.three_photoHead_channel_img:
			Intent three=new Intent(getActivity(),UserPagerActivity.class);
			log.e("lucifer", "userId0=="+seekChatBean.getMembers().get(2).get("userId"));
			three.putExtra("userId", ""+seekChatBean.getMembers().get(2).get("userId"));
			startActivity(three);
			break;
		case R.id.four_photoHead_channel_img:
			Intent four=new Intent(getActivity(),UserPagerActivity.class);
			log.e("lucifer", "userId0=="+seekChatBean.getMembers().get(3).get("userId"));
			four.putExtra("userId", ""+seekChatBean.getMembers().get(3).get("userId"));
			startActivity(four);
			break;
		case R.id.five_photoHead_channel_img:
			Intent five=new Intent(getActivity(),UserPagerActivity.class);
			log.e("lucifer", "userId0=="+seekChatBean.getMembers().get(4).get("userId"));
			five.putExtra("userId", ""+seekChatBean.getMembers().get(4).get("userId"));
			startActivity(five);
			break;
